// Kyyksin globaali liikekirjasto seed-muodossa (vrt. recipe-seed-data.mjs).
//
// Lähde: lib/demo-data.ts → defaultGlobalExercises. Tiedosto on generoitu siitä kerran ja
// pidetään jatkossa synkassa käsin — demo-data.ts on TypeScriptiä eikä sitä voi importoida
// suoraan .mjs-skripteihin, joten seed tarvitsee oman kopionsa (sama ratkaisu kuin resepteillä).
//
// Liikkeen id (ex_*) menee kannassa external_key-sarakkeeseen: training-sync.ts mapExerciseRow
// palauttaa asiakkaalle `external_key ?? id`, joten ex_*-avaimet ovat se id jonka ohjelmat näkevät.
