...
describe("getAdminOverviewAthleteGroups", () => {
  it("separates invited athletes from active athletes for admin overview cards", () => {
    ...
    expect(groups.athletes.map((user) => user.id)).toEqual(["athlete_active", "athlete_invited", "athlete_independent"]);
    expect(groups.activeAthletes.map((user) => user.id)).toEqual(["athlete_active", "athlete_independent"]);
