...
describe("shouldPreserveStoredSessionDuringSupabaseBootstrap", () => {
  ...
  it("keeps locally entered set inputs for an in-progress workout while taking server structure changes", () => {
    const state = cloneDemoState();
    const localSession: WorkoutSession = {
      id: "session_local",
      scheduledWorkoutId: "workout_local",
    ...
    // Taustasnapshot on UUDEMPI ja kantaa vanhaa esitäyttöä (revert) + lisää sarjan 2.
    const nextState = reconcileSupabaseVisibleState(state, {
      users: state.users,
      bodyMeasurements: state.bodyMeasurements,
      assignments: state.assignments,
    ...
    // Paikalliset syötetyt arvot säilyvät (eivät revertoidu itsestään)...
    expect(nextState.sessions[0]?.setLogs[0]).toMatchObject({
      actualReps: 8,
      actualLoad: 120,
      done: true,
