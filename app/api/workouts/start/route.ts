...
export async function POST(request: Request) {
  ...
  if (!user) {
    return NextResponse.json({ message: "Kirjaudu sisään ennen harjoituksen käynnistystä." }, { status: 401 });
  }
  ...
  if (!parsed.success) {
    return NextResponse.json({ message: "Virheellinen treenipyyntö." }, { status: 400 });
  }
  ...
  return NextResponse.json({ ok: true, scheduledWorkoutId: result.scheduledWorkoutId });
}
